[**ContextAI SDK**](../../README.md)

***

[ContextAI SDK](../../README.md) / react/src

# react/src

@contextai/react - React Components for ContextAI Agent SDK

Headless UI components for building AI chat interfaces.

## Interfaces

- [ReasoningStep](interfaces/ReasoningStep.md)
- [UseAgentOptions](interfaces/UseAgentOptions.md)
- [UseChatOptions](interfaces/UseChatOptions.md)
- [UseAgentStreamOptions](interfaces/UseAgentStreamOptions.md)
- [UseAgentReturn](interfaces/UseAgentReturn.md)
- [UseChatReturn](interfaces/UseChatReturn.md)
- [UseAgentStreamReturn](interfaces/UseAgentStreamReturn.md)
- [Message](interfaces/Message.md)
- [ExpandableAriaProps](interfaces/ExpandableAriaProps.md)
- [UseAnnouncerOptions](interfaces/UseAnnouncerOptions.md)
- [UseAnnouncerReturn](interfaces/UseAnnouncerReturn.md)

## Type Aliases

- [MessageRole](type-aliases/MessageRole.md)
- [LiveRegionPoliteness](type-aliases/LiveRegionPoliteness.md)

## Variables

- [VERSION](variables/VERSION.md)
- [srOnlyStyles](variables/srOnlyStyles.md)
- [A11Y\_KEYS](variables/A11Y_KEYS.md)
- [ChatWindow](variables/ChatWindow.md)

## Functions

- [useAgent](functions/useAgent.md)
- [useAgentStream](functions/useAgentStream.md)
- [useChat](functions/useChat.md)
- [generateMessageId](functions/generateMessageId.md)
- [generateA11yId](functions/generateA11yId.md)
- [getExpandableAriaProps](functions/getExpandableAriaProps.md)
- [isActivationKey](functions/isActivationKey.md)
- [useAnnouncer](functions/useAnnouncer.md)
- [announceToScreenReader](functions/announceToScreenReader.md)
- [getFocusableElements](functions/getFocusableElements.md)
- [useFocusTrap](functions/useFocusTrap.md)
- [useFocusReturn](functions/useFocusReturn.md)
- [useAutoFocus](functions/useAutoFocus.md)

## References

### ChatWindowProps

Renames and re-exports [ChatWindow](variables/ChatWindow.md)

***

### MessageList

Renames and re-exports [ChatWindow](variables/ChatWindow.md)

***

### MessageListProps

Renames and re-exports [ChatWindow](variables/ChatWindow.md)

***

### MessageInput

Renames and re-exports [ChatWindow](variables/ChatWindow.md)

***

### MessageInputProps

Renames and re-exports [ChatWindow](variables/ChatWindow.md)

***

### ReasoningTrace

Renames and re-exports [ChatWindow](variables/ChatWindow.md)

***

### ReasoningTraceProps

Renames and re-exports [ChatWindow](variables/ChatWindow.md)

***

### TraceStep

Renames and re-exports [ChatWindow](variables/ChatWindow.md)

***

### ThoughtStep

Renames and re-exports [ChatWindow](variables/ChatWindow.md)

***

### ActionStep

Renames and re-exports [ChatWindow](variables/ChatWindow.md)

***

### ObservationStep

Renames and re-exports [ChatWindow](variables/ChatWindow.md)

***

### TraceStepProps

Renames and re-exports [ChatWindow](variables/ChatWindow.md)

***

### ThoughtStepProps

Renames and re-exports [ChatWindow](variables/ChatWindow.md)

***

### ActionStepProps

Renames and re-exports [ChatWindow](variables/ChatWindow.md)

***

### ObservationStepProps

Renames and re-exports [ChatWindow](variables/ChatWindow.md)
